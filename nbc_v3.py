...
"""def num_of_class(fp, num_records):
    classes_list = []
    for record in fp:
        if not record.startswith("//"):
            record = record.split()
            classes_list.append(record[num_records])
    num_class = len(set(classes_list))
    return num_class"""
...
num_class, predict_set = num_of_class(num_attr, record_data)
print "Total Classes:", num_class
print "Class Set:", predict_set
...
if prob_of_class[0] > prob_of_class[1]:
    print "Prediction: ", predict_list[0]
else:
    print "Prediction: ", predict_list[1]
